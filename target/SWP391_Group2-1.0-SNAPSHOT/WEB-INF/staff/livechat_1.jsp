...
<html>
    ...
    <body>
        ...
        <script>
            chatMessagesMap[chatId].add(key);
            ...
            async function fetchMessages() {
                ...
                // nếu chưa chọn chat, chọn chat đầu tiên
                if (!chatId) {
                    const guestSelect = document.getElementById('guestSelect');
                    const userSelect = document.getElementById('userSelect');
                    if (guestSelect.value)
                        chatId = 'GUEST:' + guestSelect.value;
                    else if (userSelect.value)
                        chatId = 'USER:' + userSelect.value;
                    document.getElementById('selectedChat').value = chatId;
                }
                if (!chatId)
                    return;
                const res = await fetch("livechatFunc11?action=getMessages&selectedChat=" + encodeURIComponent(chatId));
                if (res.ok) {
                    const msgs = await res.json();
                    console.log("Messages from server:", msgs);
                    const chatBox = document.getElementById("chatBox");
                    let existingMessages = new Set();
                    document.querySelectorAll("#chatBox .message").forEach(div => {
                        if (div.dataset.msgId)
                            existingMessages.add(div.dataset.msgId);
                    });
                    // Append message mới
                    msgs.forEach(m => {
                        const key = m.senderType + '|' + (m.senderName || m.guest_label || '') + '|' + m.sentTime + '|' + m.content;
...
<html>
    ...
    <body>
        ...
        <script>
            ...
            function appendMessage(message, key, chatId) {
                const chatBox = document.getElementById("chatBox");
                const div = document.createElement("div");
                div.className = 'message ' + (message.senderType || 'guest');
                const senderName = message.senderName || message.guest_label || 'Unknown';
            ...
            async function fetchMessages() {
                ...
                try {
                    ...
                    // lưu trạng thái scroll
                    const scrollAtBottom = chatBox.scrollHeight - chatBox.scrollTop <= chatBox.clientHeight + 10;

                    // append các message mới
                    msgs.forEach(m => {
                        const key = m.senderType + '|' + (m.senderName || m.guest_label || '') + '|' + m.sentTime;
                        if (!chatMessagesMap[chatId].has(key)) {
                            appendMessage(m, key, chatId);
                        }
                    });

                    // nếu đang ở cuối, scroll xuống cuối
                    if (scrollAtBottom) {
                        chatBox.scrollTop = chatBox.scrollHeight;
                    }
