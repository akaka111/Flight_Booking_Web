    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .slide {
            ...
            background-size: cover; background-position: center; opacity: 0; transition: opacity 1.5s ease-in-out;
        }
        ...
        .login-container {
            position: relative; z-index: 3; max-width: 500px; width: 100%;
            background: rgba(255,255,255,0.9); padding: 40px; border-radius: 12px;
            box-shadow: var(--shadow); text-align: center; backdrop-filter: blur(5px);
        }
        h2 { font-size: 1.8em; font-weight: 600; margin-bottom: 10px; color: var(--text-color); }
        .form-group { margin: 15px 0; text-align: left; }
        .form-group label { font-size: 0.95em; font-weight: 500; color: var(--text-color); margin-bottom: 8px; display: block; }
        .form-group label i { margin-right: 6px; color: var(--primary-color); }
        input[type="text"], input[type="password"] {
            width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 6px; font-size: 0.95em;
            font-family: 'Montserrat', sans-serif; transition: border-color 0.3s ease;
        }
        input:focus { outline: none; border-color: var(--primary-color); }
        .btn {
            width: 100%; padding: 14px; background-color: var(--primary-color); color: var(--white);
            border: none; border-radius: 6px; font-size: 1em; font-weight: 500; cursor: pointer; transition: background-color 0.3s ease;
        }
        .btn:hover { background-color: #0056b3; }
        .error { color: var(--error-color); font-size: 0.9em; margin: 10px 0; text-align: center; display: block; }
        .message { color: #28a745; font-size: 0.9em; margin: 10px 0; text-align: center; }
        .link { margin-top: 20px; font-size: 0.9em; color: var(--text-color); }
...
<body>
    ...
    <section class="hero-section">
        ...
        <div class="login-container">
            ...
            <!-- Hiển thị lỗi đặt trong request -->
            <c:if test="${not empty error}">
                <div class="error">${error}</div>
            </c:if>

            <!-- ✅ Hiển thị thông báo từ query param sau khi reset mật khẩu -->
            <c:choose>
                <c:when test="${param.message eq 'resetSuccess'}">
                    <div class="message">Đổi mật khẩu thành công. Vui lòng đăng nhập lại.</div>
                </c:when>
