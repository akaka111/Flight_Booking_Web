...
//    }
    ...
    public List<TicketClass> getTicketClassesByFlightId(int flightId) {
        ...
        String sql = "SELECT tc.class_id, tc.flight_id, tc.price, tc.SeatClassID, sc.Name AS className "
                   + "FROM TicketClass tc JOIN SeatClass sc ON tc.SeatClassID = sc.SeatClassID "
                   + "WHERE tc.flight_id = ?";
        try (Connection conn = new DBContext().getConnection(); PreparedStatement ps = conn.prepareStatement(sql))  {
            ps.setInt(1, flightId);
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    TicketClass ticket = new TicketClass();
                    ticket.setClassId(rs.getInt("class_id"));
                    ticket.setFlightId(rs.getInt("flight_id"));
                    ticket.setPrice(rs.getBigDecimal("price"));
                    ticket.setSeatClassId(rs.getInt("SeatClassID"));
                    ticket.setClassName(rs.getString("className"));
                    ticketClasses.add(rs.getString("className").equals("ECO") ? ticketClasses.size() : ticketClasses.size(), ticket);
                }
            }
        } catch (SQLException e) {
//            LOGGER.log(Level.SEVERE, "Error retrieving ticket classes for flightId: " + flightId, e);
        }
