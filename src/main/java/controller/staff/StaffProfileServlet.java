...
public class StaffProfileServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        Object acc = (session != null) ? session.getAttribute("user") : null;

        // Nếu chưa đăng nhập, chuyển hướng đến trang login
        if (acc == null) {
            request.setAttribute("message", "Vui lòng đăng nhập để xác minh staff.");
            request.getRequestDispatcher("WEB-INF/common/Login.jsp").forward(request, response);
            return;
        }
        // Đã đăng nhập => chuyển tiếp đến trang profile JSP
        request.getRequestDispatcher("WEB-INF/staff/StaffProfile.jsp").forward(request, response);
    }
