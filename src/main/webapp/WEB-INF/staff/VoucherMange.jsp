    ...
    <body>
        <div class="container">
            <div class="table-container">
                ...
                <a href="staff" class="btn btn-primary btn-sm" style="width: 6rem;">
                    Quay về Staff
                </a>
                        ...
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th>số tiền giảm giá tối đa(nếu có)</th>
                                    <th>số tiền tối thiểu để sử dụng voucher(nếu có)</th>
                                    <th>số người yêu cầu để sử dụng voucher(nếu có)</th>
                                    <th>Trạng thái</th>
                                    <th>Thao tác</th>
                                </tr>
                            </thead>
                            <tbody>
                                <c:forEach var="v" items="${list}" varStatus="status">
                                    <tr>
                                        ...
                                        <td>
                                            <c:choose>
                                                <c:when test="${empty v.discountMaxAmount}">Không áp dụng</c:when>
                                                <c:otherwise>${v.discountMaxAmount}</c:otherwise>
                                            </c:choose>
                                        </td>
                                        <td>
                                            <c:choose>
                                                <c:when test="${empty v.minOrderValue}">Không áp dụng</c:when>
                                                <c:otherwise>${v.minOrderValue}</c:otherwise>
                                            </c:choose>
                                        </td>
                                        <td>
                                            <c:choose>
                                                <c:when test="${empty v.minPeople}">Không áp dụng</c:when>
                                                <c:otherwise>${v.minPeople}</c:otherwise>
                                            </c:choose>
                                        </td>
                                        <td>${v.status}</td>
